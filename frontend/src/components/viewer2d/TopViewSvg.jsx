...
export default function TopViewSvg({ layout, selectedId, onSelect }) {
  ...
  const SVG_W  = 600;
  const SVG_H  = 500;
  const scale  = Math.min((SVG_W - PADDING * 2) / land.width, (SVG_H - PADDING * 2) / land.depth);
    ...
    <svg viewBox={`0 0 ${SVG_W} ${SVG_H}`} style={{ width: '100%', height: '100%', display: 'block' }}>
      ...
      {zones.map(z => (
        <rect key={z.id}
          ...
          strokeWidth="0.6" strokeDasharray="4 3" rx="2"
        />
      ))}

      {/* House */}
      <rect x={wx(house.x)} y={wy(house.y + house.depth)}
        width={house.width * scale} height={house.depth * scale}
        fill="#1e293b" stroke="#94a3b8" strokeWidth="1.5" rx="2"
      />
      <text x={wx(house.x + house.width / 2)} y={wy(house.y + house.depth / 2) + 4}
        ...
        return (
          <g key={pw.id}>
            <polyline points={pts} fill="none"
              stroke={MATERIAL_COLORS[pw.material] || '#4a4540'}
              strokeWidth={pw.width * scale} strokeLinecap="round" strokeLinejoin="round" opacity="0.85"
            />
            <polyline points={pts} fill="none"
      ...
      {objects.map(obj => {
        ...
        return (
          <g key={obj.id} onClick={(e) => { e.stopPropagation(); onSelect?.(obj.id); }} style={{ cursor: 'pointer' }}>
            ...
            {w > 18 && h > 10 && (
              <text x={x + w / 2} y={y + h / 2 + LABEL_FONT / 3}
                textAnchor="middle" fill={color} fontSize={LABEL_FONT}
